...
Она берет следующие параметры :
 ...
 --dpi-desync-retrans=0|1               ; 1 : дропать оригинальный пакет с данными, заменяя его подделкой, заставляя ОС выполнять ретрансмиссию через 0.2 сек
...
* badsum не сработает, если ваше устройство за NAT, который не пропускает пакеты с инвалидной суммой.
  ...
  Если nfqws работает на роутере, то не обязательно выключать nf_conntrack_checksum. Фейковый пакет не проходит NAT, он идет через OUTPUT.
...
* TTL казалось бы - лучший вариант, но он требует индивидуальной настройки по каждого провайдера. Если DPI находится дальше локальных
...
В списке могут присутствовать и записи вида ip/prefix. Такие записи выкидываются в stdout без изменений.
...
Без рута забудьте про режим transparent proxy : будет работать только режим --socks.
